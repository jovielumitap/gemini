...
class FloorList extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            noContentFoundMessage: "No floor found in this building",
            alertMessage: "",
            showMessage: false,
            selectedSectionId: 1,
            drawerState: false,
    ...
    render() {
        ...
        const dataList = searchKey === "" ? floors : this.filterData(floors, searchKey);
        return (
...
const mapStateToProps = ({settings, floor}) => {
    const {width} = settings;
    const {floors} = floor;
    return {width, floors};
};
export default withRouter(connect(mapStateToProps)(FloorList));
