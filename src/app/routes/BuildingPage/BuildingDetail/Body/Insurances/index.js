...
class Insurances extends Component {
  ...
  onEdit = (item) => {
    this.setState({selectedItem: item, addNew: true});
  };
  onSave = (data) => {
    this.setState({addNew: false});
    this.props.dispatch(showLoader());
    this.props.dispatch(createNewInsurance(data));
  };
  onUpdate = (body) => {
    this.setState({addNew: false, selectedItem: {}});
    this.props.dispatch(showLoader());
    this.props.dispatch(updateInsurance(body.id, body));
  };
  onDelete = () => {
    this.setState({addNew: false, selectedItem: {}});
  };
  ...
  componentDidMount() {
    this.props.dispatch(fetchInsurances(this.getBodyId()));
  }
  getBodyId = () => {
  ...
  constructor(props) {
    super(props);
    this.state = {
      noContentFoundMessage: "No insurance found",
      alertMessage: "",
      showMessage: false,
      drawerState: false,
      searchKey: "",
...
const mapStateToProps = ({ settings, insurance }) => {
  const { width } = settings;
  const { insurances } = insurance;
  return { width, insurances };
};
